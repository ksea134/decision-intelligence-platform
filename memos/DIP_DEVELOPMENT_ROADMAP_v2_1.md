...
| **2.1** | **2026-03-18** | **「Vertex AI Vector Search」→「Vertex AI Search」に統一。Phase 3のコード例・GCPリソース構成を更新** |
...
| 3 | Vertex AI Search統合 | 2週間 | ⬜ 未着手 | RAG/Grounding基盤 |
...
│   ├── search_client.py                # Vertex AI Search（Phase 3）
...
### 概要
Vertex AI Searchを導入し、非構造化データ（GCS上のドキュメント）と過去の質問・回答を長期記憶として保持。RAG/Grounding機能により回答精度を向上させる。

### 開発理由
- 過去の意思決定事例を活用した回答生成
- GCS上の非構造化データ（txt/md/pdf）の意味検索
- Gemini APIのGrounding機能との統合
- マネージドサービスによる運用負荷軽減
...
| 機能 | 説明 | 技術要素 |
|------|------|----------|
| **F3-1: Data Store作成** | GCSドキュメントのインデックス化 | Vertex AI Search Data Store |
| **F3-2: Search App作成** | 検索アプリケーションの構築 | Vertex AI Search App |
| **F3-3: 意味検索** | 自然言語によるドキュメント検索 | Discovery Engine API |
| **F3-4: Grounding統合** | Gemini APIのGroundingソースとして利用 | Grounding with Vertex AI Search |
| **F3-5: メタデータ管理** | 企業ID、日時、カテゴリ等の付与 | Metadata Schema |

### 技術的説明
```python
# infra/search_client.py
...
class GroundingService:
    """Gemini APIとVertex AI Searchの統合"""
    ...
    def get_grounding_config(self) -> dict:
        """Gemini API呼び出し用のGrounding設定を返す"""
        return {
            "grounding_source": {
                "type": "VERTEX_AI_SEARCH",
                "vertex_ai_search_data_store": self.grounding_source
            }
        }
```
...
│  │ - Source: GCS (gs://dip-documents/)                 │  │
│  │ - Auto-sync: Enabled                                │  │
│  │ - Chunking: 500 tokens (default)                    │  │
│  └─────────────────────────────────────────────────────┘  │
│                                                           │
│  ┌─────────────────────────────────────────────────────┐  │
...
│  │ - Type: Search                                      │  │
│  │ - Data Store: dip-knowledge-store                   │  │
│  │ - Features: Summarization, Snippets, Citations      │  │
│  └─────────────────────────────────────────────────────┘  │
│                                                           │
│  ┌─────────────────────────────────────────────────────┐  │
│  │ Grounding Integration                               │  │
│  │ - Gemini API → Vertex AI Search Data Store          │  │
