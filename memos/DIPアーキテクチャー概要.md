...
Decision Intelligence Platform（DIP）は、**企業のデータ資産を理解し、適切な権限の下で、適切なデータを選び、専門的な分析を提供するプラットフォーム**。ユーザーが自然言語で質問すると、Dataplexデータカタログから関連データを特定し、BigQuery上の構造化データやGCS上のドキュメントをもとに、分析・比較・予測などの回答を生成する。

### DIPの本質的価値（2026-03-22定義）

DIPは単なるチャットボットではない。以下の3つが本質的価値:
1. **データ資産の理解**: Dataplex Data Catalogを通じて、テーブルの「意味」を理解して適切なデータを選択
2. **権限に基づくアクセス制御**: Dataplex Data Governanceにより、ユーザーごとに見えるデータを制限
3. **専門的な分析**: AIエージェント（要因分析/比較/予測/汎用）が質問に応じた専門的回答を生成

詳細設計 → `memos/開発検討項目.md` 検討006
