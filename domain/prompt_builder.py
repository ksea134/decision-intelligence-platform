...
def _build_viz_tag_instruction() -> str:
    return (
        ...
        '- type="mermaid" : フローチャート・構造図（手順・関係性・プロセスの可視化に最適）\n\n'
        "■ Mermaidフローチャートの形式:\n"
        '<viz type="mermaid" title="フローのタイトル">\n'
        "graph TD\n"
        '    A["ステップ1"] --> B["ステップ2"]\n'
        '    B --> C["ステップ3"]\n'
        "</viz>\n\n"
        "■ Mermaidのルール:\n"
        "- graph TD（上→下）またはgraph LR（左→右）を使うこと\n"
        "- ノード名は日本語で、必ずダブルクォート付き角括弧で囲むこと（例: A[\"開始\"]）\n"
        "- 手順の説明、業務フロー、意思決定プロセスなどに積極的に使うこと\n\n"
        "■ ルール:\n"
        "- チャートは回答の流れの中に自然に配置すること（末尾にまとめない）\n"
        "- labelsとdataの要素数は必ず一致させること（bar/line/pie）\n"
        "- dataの値は数値のみ（文字列は不可）（bar/line/pie）\n"
        "- 1回答あたりチャートは最大3つまで\n"
